### Added
- SDK: `PodStartError` (a `LiumError`) with `pod_id`, `pod`, `status` and `history`. `Lium.wait_ready()` raises it as soon as the pod reports a terminal status (`FAILED`, `STOPPED`, `ERROR`, `TERMINATED`, `DELETED`, `REMOVED`, `CANCELLED`, and the backend's own dead-end names `CREATION_FAILED`, `BROKEN`, `REBOOT_FAILED`, `DELETING`), disappears from the pod list after having been seen, or is never listed in three consecutive polls. A timeout while the pod is still starting keeps returning `None`.
