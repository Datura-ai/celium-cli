...
def _post_rent_failure(exc: Exception, billing_pod: dict, doing: str, note: str = "") -> CliFailure:
    ...
    if isinstance(exc, LiumError):
        return sdk_error_failure(exc, data=billing_pod)
...
def up_command(
        ...
        if strict_gpus and result.data.get("mismatch"):
            try:
                ...
                raise CliFailure(
                    ...
                    EXIT_GENERAL_ERROR,
                    data={**result.data, **billing_pod, "pod_removed": False},
                )
    ...
    if json_output:
        # The same pod view as `lium ps --format json`, so one parser serves both; the
        # SSH session and the log stream are for a person, so --json ends here.
