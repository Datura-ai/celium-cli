...
- `lium up [NODE_ID]` - Create a pod (use node ID or filters like `--gpu`, `--count`, `--country`); a row number is the last `lium ls` in the same shell, for 10 minutes — a listing another shell wrote is refused. Use the huid in scripts.
