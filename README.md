...
- `lium up [NODE_ID]` - Create a pod (use node ID or filters like `--gpu`, `--count`, `--country`; `--json` prints the ready pod as JSON instead of opening SSH)
...
- `lium rm <POD>` - Remove/stop a pod (`--name-only` to refuse `lium ps` row numbers in scripts; `--json` prints what was removed)
...
`ls`, `ps`, `templates`, `balance` and `describe` all accept `--format json` (and `--json`); `up` and `rm` accept `--json`. All of them print a JSON error envelope on stderr when the command fails, so the same flag works across commands in scripts.
